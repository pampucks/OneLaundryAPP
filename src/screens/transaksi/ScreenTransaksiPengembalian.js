...
const ScreenTransaksiPengembalian = ({ navigation }) => {
    ...
    <>
      <Provider>
        ...
        <List.Section style={{ paddingHorizontal: 16 }}>
          <List.Item
            title="Pengembalian Cucian"
            onPress={() => setVisible(true)}
            left={() => (
              <>
                {!complete && <ActivityIndicator animating={!complete} />}
                {complete && <List.Icon icon="hand-coin-outline" />}
              </>
            )}
          ></List.Item>
        </List.Section>
...
// export default ScreenTransaksiPengembalian;
